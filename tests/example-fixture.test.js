...
describe("tiny app example", () => {
  it("runs tests before and after framework bootstrap", () => {
    const target = fs.mkdtempSync(path.join(os.tmpdir(), "claude-tiny-app-"));
    try {
      ...
      // Use bootstrap.js (Node-based) which copies scripts + schemas + templates
      // and adds npm script shims to the target's package.json
