...
describe("helper scripts", () => {
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), "claude-helper-"));
  });
  ...
  it("roadmap shows no-roadmap message when file is missing", () => {
    const result = spawnSync(process.execPath, [
      path.join(ROOT, "scripts", "roadmap.js"),
    ], {
      cwd: tmp,
      encoding: "utf8",
    });
    ...
    assert.equal(result.status, 0);
    assert.match(result.stdout, /No roadmap found/);
  });

  it("summary writes pipeline summary file", () => {
    fs.mkdirSync(path.join(tmp, "pipeline", "gates"), { recursive: true });

    const result = spawnSync(process.execPath, [
      path.join(ROOT, "scripts", "summary.js"),
    ], {
      cwd: tmp,
      encoding: "utf8",
    });
