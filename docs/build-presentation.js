...
/** Slide 4: Three pillars — Commands, Skills, Agents. */
function slideThreePillars(pres, I) {
  const s = pres.addSlide();
  s.background = { color: C.off_white };
...
/** Slide 5: When to Use What — situation/command table. */
function slideWhenToUse(pres) {
  const s = pres.addSlide();
  s.background = { color: C.off_white };
...
/** Slide 9: Customizing for Your Stack — extensions and examples. */
function slideCustomizing(pres) {
  const s = pres.addSlide();
  s.background = { color: C.off_white };
...
/** Slide 11: The implement skill — 3-step workflow. */
function slideImplementSkill(pres, I) {
  const s = pres.addSlide();
  s.background = { color: C.off_white };
...
/** Slide 12: Pre-merge review — four check cards. */
function slidePreMergeReview(pres, I) {
  const s = pres.addSlide();
  s.background = { color: C.off_white };
...
/** Slide 14: Pipeline stages + virtual team table. */
...
/** Slide 17: Health Check & Roadmap dashboard. */
function slideHealthRoadmap(pres, I) {
  const s = pres.addSlide();
  s.background = { color: C.off_white };
...
/** Slide 18: Getting Started — 5 numbered steps. */
...
/** Build the full 18-slide presentation deck. */
async function build() {
  ...
  slideImplementSkill(pres, I);
  // 12. Pre-Merge Review
  slidePreMergeReview(pres, I);
  ...
  slideHealthRoadmap(pres, I);
  // 18. Getting Started
  slideGettingStarted(pres, I);

  const outArg = process.argv.find(a => a.startsWith("--out="));
  const outPath = outArg ? outArg.slice("--out=".length) : "claude-dev-team-lifecycle.pptx";
