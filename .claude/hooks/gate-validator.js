 ...
 *
 * Also exports REQUIRED_FIELDS for consumption by the gate-schema
 * consistency test.
 */
...
const REQUIRED_FIELDS = ["stage", "status", "agent", "timestamp", "blockers", "warnings"];
...
function main() {
  try {
    ...
    const missing = REQUIRED_FIELDS.filter(k => !(k in gate));
    if (missing.length > 0) {
    ...
    // Never block the pipeline on our own bug. Surface the error as a warning
    // and exit 0 so the orchestrator continues.
    console.error(`[gate-validator] WARN: internal error — ${e.message}`);
    process.exit(0);
  }
}
